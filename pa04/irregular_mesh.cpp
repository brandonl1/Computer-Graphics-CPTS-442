...
const void IrregularMesh::render(void)
{
    ...
    //
    // - Remove the (hack) to save/restore a copy of
    //   `vertexPositions[]` in `savedPositions[]` (or whatever you
    //   called it).
    //
    // - Remove the updateBuffers() call. It will now be called in the
    //   constructor.
    //
    // - Remove the transform of `vertexPositions[]`. (That's in the vertex
    //   shader now.)
    //
    // Be sure to keep the renderTriangles() call. You still want to
    // do that!
    //
...
void IrregularMesh::updateBuffers(void)
{
    ...
    //
    CHECK_GL(glBindBuffer(GL_ARRAY_BUFFER, vertexPositionsBufferId));
    CHECK_GL(glBufferData(GL_ARRAY_BUFFER,  nVertices* sizeof(vertexPositions[3]),
