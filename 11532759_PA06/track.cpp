...
void Track::addSupports(const double maxHeight)
{
   ...
   // trackTrigonometricCurve= new TrigonometricCurve(mag, freq, phase, offset,nRailSegments );
    ...
    int i = 0;
    int nJ = 0;
  //  Point3 vertexPositions[nSupports][2];
    while(i < nSupports){
    	//vertexPositions[i][0] = point;
    	//vertexPositions[i][1] = point;
    	u = (double)i/(double)nSupports;
    	Point3 point =(*guideCurve)(u, NULL);
    	Point3 ground(point.g.x, point.g.y, 0);
    	//point.g.z = 0;
    	LineSegment *lSegment = new LineSegment(ground, point, Vector3(1,0,0));
	nJ = max(2.0, round(point.g.z * 10 / maxHeight));
    	//point.g.z = 0;
	supportTubes.push_back(new Tube(lSegment, radius, nTheta, nJ, false));
  	i++;
    }
}
                    ...
                    const Curve *rightRailCurve)
{
    ...
    //
    int ties = nSupports * nTiesPerSupport;
    int i = 0;
    
    while(i < ties){
    	double u = (double)i / (double)ties;
    	Point3 leftCurve = (*leftRailCurve)(u, NULL);
    	Point3 rightCurve = (*rightRailCurve)(u, NULL);
    	LineSegment *lSegment = new LineSegment(leftCurve, rightCurve, Vector3(0,0,1));
     	tieTubes.push_back(new Tube(lSegment, radius, nTheta, 4, false));
    	i++;
    }
...
void Track::setGuideCurve(void)
{
    ...
    //
     ...
     guideCurve = new TrigonometricCurve(mag, freq, phase, offset, Vector3(0,0,1));
}
...
Track::Track(void) : SceneObject()
{
    ...
    setGuideCurve();
    //double u = 0.1;
    OffsetCurve *leftRailCurve = new OffsetCurve( guideCurve, (0.5 * railSep) * Vector3(1,0,0), Vector3(0,0,1) );
    leftRailTube = new Tube(leftRailCurve, radius, nTheta, nRailSegments, true);
    OffsetCurve *rightRailCurve = new OffsetCurve( guideCurve, (-0.5 * railSep) * Vector3(1,0,0), Vector3(0,0,1) );
    rightRailTube = new Tube(rightRailCurve, radius, nTheta, nRailSegments, true);
    addTies(leftRailCurve, rightRailCurve);
   //leftRailCurve->addSupports((2 * mag.g.z) + offset.g.z);
    //rightRailCurve->addSupports((2 * mag.g.z) + offset.g.z);
    addSupports((2.0 * mag.g.z) + offset.g.z);
    //Point3 vertexPositions[nSupports][2];
    //int i = 0;
    ...
    //	point.g.z = 0;
    //	vertexPositions[i][1] = point;
 	//supportLines = new Lines(vertexPositions, nSupports);
  //	i++;
   // }
