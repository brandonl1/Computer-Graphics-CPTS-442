...
public:
    ...
    const Vector3 cross(const Vector3 &other) const
    {
        // replace the following (which permits the template to compile cleanly)
         return Vector3((g.y * other.g.z) - (g.z * other.g.y), (g.z * other.g.x) - (g.x * other.g.z), (g.x*other.g.y) - (g.y * other.g.x));
    };
};
...
public:
    ...
    int textureIndex;
    int normalIndex;
    FaceVertex(int positionIndex_, int textureIndex_, int normalIndex_)
...
public:
    ...
    Vector3 normal; // of this face (computed, not in OBJ)
