...
const void IrregularMesh::render(void)
{
    ...
    //
     ...
     if(vnai !=  NO_SUCH_ATTRIBUTE){
     	if(controller.useVertexNormals == true){
     		CHECK_GL(glBindBuffer(GL_ARRAY_BUFFER, vertexNormalBufferId));
     	}
     	else if(controller.useVertexNormals == false){
...
void IrregularMesh::updateBuffers(void)
{
    ...
    for (int iFace = 0; iFace < ((sizeof(faceNormals) * nVertices) / sizeof(faceNormals[0])); iFace++) {
    	int temp = iFace * 3;
        faceNormalOfVertex[temp] = faceNormals[iFace];
        faceNormalOfVertex[temp + 1] = faceNormals[iFace];
        faceNormalOfVertex[temp + 2] = faceNormals[iFace];

    }
    CHECK_GL(glBindBuffer(GL_ARRAY_BUFFER, faceNormalBufferId));
    CHECK_GL(glBufferData(GL_ARRAY_BUFFER,  nVertices * sizeof(faceNormalOfVertex[0]),
