...
void Track::addSupports(const double maxHeight, const Ground *ground)
{
  ...
  //  Point3 vertexPositions[nSupports][2];
    while(i < nSupports){
    	...
    	Point3 ground_(point.g.x, point.g.y, ground->height(point.g.x, point.g.y));
    	//point.g.z = 0;
    	LineSegment *lSegment = new LineSegment(ground_, point, Vector3(1,0,0));
             ...
             const Ground *ground) : SceneObject()
{
   ...
   //leftRailCurve->addSupports((2 * mag.g.z) + offset.g.z);
    ...
    addSupports((2.0 * mag.g.z) + offset.g.z, ground);
    //Point3 vertexPositions[nSupports][2];
    //int i = 0;
